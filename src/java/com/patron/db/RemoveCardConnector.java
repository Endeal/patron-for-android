...
public class RemoveCardConnector extends AsyncTask<Context, Void, String>
{
	private Loadable activity;
	private Funder funder;

	public RemoveCardConnector(Loadable activity, Funder funder)
	{
	...
	{
		String cardHref = funder.getHref();
		Map<String, Object> response = null;
		try
		{
			// Remove the card from the customer.
			HttpClient client = new DefaultHttpClient();
        	HttpPost post = new HttpPost(ListLinks.LINK_REMOVE_CARD);
	...
	protected void onPostExecute(String result)
	{
		...
		if (result.equals("1"))
		{
			activity.message("Removed card.");
			activity.update();
		}
		else
		{
			activity.message("Failed to remove card.");
