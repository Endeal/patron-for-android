...
public class Product
{
	...
	{
		BRANDY, GIN, HOUSE_LIQUOR, NON_ALCOHOLIC, NONE, OTHER, TEQUILA,
		VODKA, WHISKEY;
	}
	public static enum Type
	{
		DOMESTIC_BEER, IMPORTED_BEER, COCKTAIL, MARTINI, MIXED, NONE,
		OTHER, PREMIUM, SHOT, SODA, WINE;
	}
	...
	public Product(String id, String name, Type type, Alcohol alcohol, float price)
	{
		this.id = id;
		this.name = name;
		...
		this.price = price;
		topOrder = false;
		int counter = Globals.getFavoriteProducts().size();
		for (int i = 0; i < counter; i++)
		{
			if (Globals.getFavoriteProducts().get(i).getId() == id)
			{
				topOrder = true;
				break;
			}
	...
	public Product(String id, String name, String type, String alcohol, float price)
	{
		this.id = id;
		this.name = name;
		...
		this.price = price;
		topOrder = false;
		if (Globals.getFavoriteProducts() != null)
		{
			int counter = Globals.getFavoriteProducts().size();
			for (int i = 0; i < counter; i++)
			{
